...
int main(){
    ...
    int plainTextLength = sizeof(plainText);
    unsigned char key[] = "Hardcoded";
    unsigned char* encrypted = aes->EncryptECB(plainText,16,key);
    unsigned char* decrypted = aes->DecryptECB(encrypted,16,key);
    cout<<decrypted;
}